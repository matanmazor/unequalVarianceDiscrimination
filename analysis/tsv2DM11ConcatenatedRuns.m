...
% modulator. baseline is discrimination and tilt recognition
...
for i_s = which_subjects
    ...
    if sum(which_blocks(i_s,:))>1
        ...
        for i_p = 1:6
            ...
            pmod(1).name{i_p} = ['Y',num2str(i_p)];
            pmod(1).param{i_p} = [];
            pmod(1).poly{i_p} = 1;
            
            pmod(1).name{6+i_p} = ['N',num2str(i_p)];
            pmod(1).param{6+i_p} = [];
            pmod(1).poly{6+i_p} = 1;
        end
        ...
        for i_r = 1:numel(relevant_runs)
            ...
            for event = 1:length(table.onset)
                ...
                if ismember(event_name, {'CC','AC','CA','AA','YY','YN','NY','NN', ...
                    ...
                    p_col = 6*strfind('YN',event_name(2))+confidence_level-6;
