...
base_dir = fullfile('..','analyzed','DM6_unsmoothed','group','rsa_gm','euclidean');
...
for i_RDM = 1:length(RDMs)
    fig = figure;
    hold on;
    h = bar(1:6,means(i_RDM,:),'FaceColor','flat');
    h.CData=repmat([1,1,1],6,1);
    errorbar(1:6,means(i_RDM,:),errors(i_RDM,:),errors(i_RDM,:),'lineStyle','none','color','black');
    xlim([0.5,6.5]); 
    ylim([-0.1,0.1]);
    ...
    s.Width = 12;
    s.Height = 8;
    hgexport(fig,fullfile('figures',[RDMs{i_RDM},'_','correlations_euclidean_gm']),s);
end