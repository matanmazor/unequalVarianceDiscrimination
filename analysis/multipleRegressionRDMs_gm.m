function [] = multipleRegressionRDMs_gm(project_params,which_subjects,metric)

%global median for R2
...
ROIs = {'FPl','FPm','BA46','vmPFC','rSTS','preSMA'};
...
%     hgexport(fig,fullfile('figures',[ROI,'_','coefficients_',metric]),s);
    exportgraphics(f,fullfile('figures',[ROI,'_','coefficients_',metric,'_gm.png']),'Resolution',300)
...
%     hgexport(fig,fullfile('figures',[ROI,'_','combinations_',metric]),s);
    f = gcf;
    exportgraphics(f,fullfile('figures',[ROI,'_','combinations_',metric,'_gm.png']),'Resolution',300)
...
set(gca,'xtick',0.15+(1:6),'xticklabel',ROIs);
ylabel('beta combination')
 s=hgexport('readstyle','presentation');
s.Format = 'png';
s.Width = 12;
s.Height = 6;
hgexport(fig,fullfile('figures',['all_combinations_',metric, '_gm']),s);
